...
#### Create a table for evaluation metrics of multiple methods ####
 ...
 option_list = list(
    make_option(
        ...
        default = NA,
        type = 'character',
        help = 'Path to the directory with standardised output .tsv files
...
# parse arguments 
opt = wsc_parse_args(option_list, mandatory = c("input_dir", "ref_file",
                                                "lab_cl_mapping", "output_path"))
...
.get_metrics = function(idx){
    ...
    if(!all(as.character(reference_labs_df[, barcode_ref]) == 
                         as.character(predicted_labs_df[, barcode_pred]))) {
